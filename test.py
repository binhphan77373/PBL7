...
path_video = "/home/asus/Downloads/PBL/M2_estimate_distance/Demo.avi"
# Visualize frame
...
# def calculate_distance_danger_area(camera_name: str):
#     #path_video = f"video/{camera_name}.mp4"
#     #def generate_frames():
...
#                 raw_worker_bboxes, raw_worker_labels, raw_worker_scores = mmdet3x_convert_to_bboxes_mmdet(worker_results,
#                                                                                                         worker_threshold)
#                 worker_bboxes, worker_labels, worker_scores = filter_class1(raw_worker_bboxes, raw_worker_labels,
#                                                                         raw_worker_scores, 'class_1', prefix='worker')
#                 vehicle_results = inference_detector(vehicle_mmdet_model, frame)
#                 raw_vehicle_bboxes, raw_vehicle_labels, raw_vehicle_scores = mmdet3x_convert_to_bboxes_mmdet(vehicle_results,
#                                                                                                             vehicle_threshold)
#                 vehicle_bboxes, vehicle_labels, vehicle_scores = filter_class1(raw_vehicle_bboxes, raw_vehicle_labels,
#                                                                             raw_vehicle_scores, 'class_8', prefix='vehicle')
...
#                 if elapsed_time > 28 and elapsed_time <=30:
#                     cv2.putText(frame, "Compute depth map...", (10, 120), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
#                 if elapsed_time >= 30:
...
#                         line_color = (255, 0, 0)
#                         cv2.putText(frame, "Danger! Worker too near with construction vehicle = {}".format(np.around(distance,2)), (10, 80), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
#                     else:
...
#                     cv2.putText(frame, distance_text, midpoint, cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
...
#             return StreamingResponse(calculate_distance_danger_area(), media_type="multipart/x-mixed-replace;boundary=frame")
@app.get("/calculate_distance_danger_area/{camera_name}", tags=["Distance"])
def calculate_distance_danger_area(camera_name: str):
    def generate_frames():
        cap = cv2.VideoCapture(path_video)
        ...
        while True:
            ...
            if ret:
                worker_results = inference_detector(person_mmdet_model1, frame)
                raw_worker_bboxes, raw_worker_labels, raw_worker_scores = mmdet3x_convert_to_bboxes_mmdet(worker_results, person_threshold)
                worker_bboxes, worker_labels, worker_scores = filter_class1(raw_worker_bboxes, raw_worker_labels, raw_worker_scores, 'class_1', prefix='worker')

                vehicle_results = inference_detector(vehicle_mmdet_model, frame)
                raw_vehicle_bboxes, raw_vehicle_labels, raw_vehicle_scores = mmdet3x_convert_to_bboxes_mmdet(vehicle_results, vehicle_threshold)
                vehicle_bboxes, vehicle_labels, vehicle_scores = filter_class1(raw_vehicle_bboxes, raw_vehicle_labels, raw_vehicle_scores, 'class_8', prefix='vehicle')
                ...
                object_infor = extract_bounding_boxes_and_depth(combined_bboxes, combined_labels, object_depths)
                object_distances = computer_object_distance(object_infor)
                center_points = [((box['x1'] + box['x2']) // 2, (box['y1'] + box['y2']) // 2) for box in object_infor]

                for (i, j), distance_info in object_distances.items():
                    pt1, pt2 = center_points[i], center_points[j]
                    distance = distance_info['distance']
                    line_color = (255, 0, 0) if distance < safe_distance else (0, 255, 255)
                    cv2.line(frame, pt1, pt2, line_color, 2)
                    midpoint = ((pt1[0] + pt2[0]) // 2, (pt1[1] + pt2[1]) // 2)
                    distance_text = f"{distance:.2f}"
                    cv2.putText(frame, distance_text, midpoint, cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
